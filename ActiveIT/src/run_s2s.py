...
class DataTrainingArguments:
    ...
    max_num_instances_per_test_task: int = field(
        default=100, metadata={"help": "The maximum number of instances we will consider for each test task. Will force rewrite the original 100 data"}
    )
    ...
    for_debug: Optional[bool] = field(
        default=False,
        metadata={"help": "If specified, run some code for debug"}
    )
    task_metadata_file: Optional[str] = field(
        default="task_metadata.json",
        metadata={"help": "Specify where's the task_metadata file for all tasks."}
    )
    used_label_file: Optional[str] = field(
        default=None,
        metadata={"help": "If specified, use a predicted_examples.jsonl results as the data label"}
    )
    pred_remain: Optional[bool] = field(
        default=False,
        metadata={"help": "If specified, the test tasks will be in remain_tasks.txt"}
    )
    perturb_def_drop_rate: Optional[float] = field(
        default=0.1,
        metadata={"help": "The drop rate for perturb task definition"}
    )
    perturb_def_mask: Optional[bool] = field(
        default=False,
        metadata={"help": "If specified, while dropping word for perturb task definition, add mask tokens"}
    )
...
class NITrainingArguments(Seq2SeqTrainingArguments):
    ...
    shuffle_training_dataset: bool = field(
        default=True,
        metadata={"help": "Whether to shuffle the dataset during training."},
    )
    uncertainty: Optional[str] = field(
        default=None,
        metadata={"help": "If specified, use specific uncertainty measurement. [perplexity, PI-1.1.1 to V2.2.3]"}
    )
    pred_file_prefix: Optional[str] = field(
        default="",
        metadata={"help": "Prefix for predict example file. The original predict file is predicted_examples.jsonl"}
    )
    perturb_num: Optional[int] = field(
        default=0,
        metadata={"help": "If uncertainty is PerturbInstruction(start with PI-), this will specifiy how many perturb version. "}
    )
    skip_generate: Optional[bool] = field(
        default=False,
        metadata={"help": "If specified, skip generate "}
    )
    perturb_inst: Optional[str] = field(
        default=None,
        metadata={"help": "If specified, use specific method to perturb instruction, [TE, TD, ALL, MC]"}
    )
...
def main():
    ...
    raw_datasets = load_dataset(
        ...
        perturb_inst = training_args.perturb_inst,
        perturb_num = training_args.perturb_num,
        pred_remain = data_args.pred_remain,
        perturb_def_drop_rate = data_args.perturb_def_drop_rate,
        perturb_def_mask = data_args.perturb_def_mask,
        used_label_file = data_args.used_label_file,
    )
    ...
    def compute_ni_metrics(dataset, preds, save_prefix=None):
        ...
        # results per split
        splits = [it.lower() for it in dataset["Split"]]
        result_per_split = compute_grouped_metrics(predictions=decoded_preds, references=references, groups=splits)
        result.update(result_per_split)
        # results per split + cls_gen
        spt_cg = [f"{spt.lower()}_{cg.lower()}" for spt, cg in zip(dataset["Split"], dataset["CLS_GEN"])]
        STATS = {tag: 0 for tag in list(set(spt_cg))}
        for tag in spt_cg:
            STATS[tag] += 1
        print("--------------- SPLIT and CLS_GEN Stats ---------------")
        print(STATS)
        result_per_spt_cg = compute_grouped_metrics(predictions=decoded_preds, references=references, groups=spt_cg)
        result.update(result_per_spt_cg)
        # Stats
    ...
    if training_args.do_predict:
        ...
        if not data_args.pred_remain:
            # If pred_remain, only save the predicted_examples.jsonl file
            trainer.log(metrics)
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                #     writer.write("\n".join(predictions))
                if training_args.uncertainty is not None:
                    file_prefix = f"{training_args.uncertainty}_".replace("-High", "").replace("-Low", "")
                else:
                    file_prefix = training_args.pred_file_prefix
                output_prediction_file = os.path.join(training_args.output_dir, f"{file_prefix}predicted_examples.jsonl")
                with open(output_prediction_file, "w") as fout:
                    if training_args.uncertainty is not None:
                        for example, prediction, uncertainty in zip(predict_dataset, predictions, predict_results.uncertainty):
                            example["prediction"] = prediction
                            example["uncertainty_type"] = training_args.uncertainty
                            example["uncertainty"] = float(uncertainty)
