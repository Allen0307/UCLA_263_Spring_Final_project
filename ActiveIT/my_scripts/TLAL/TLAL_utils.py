...
def augment(remain_task_list, model_mode):
    print("***** Running Augmentation *****")
    model_dict = {"a": ["wizardlm", "llama", "mistral"], "w": ["wizardlm"], "l": ["llama"], "m": ["mistral"], "g": ["gpt4o"]}
    model_list = model_dict[model_mode]
    augmented_list = []
    all_task = os.listdir("/home/allenfu/LH-263/Active-IT/natural-instructions/tasks")
    for task in remain_task_list:
        for i in range(1,6):
            for model in model_list:
                aug_task = f"{task}_{model}_{i}.json"
                if aug_task in all_task: augmented_list.append(aug_task[:-5])

    # if model_mode == "a":
    #     return split_list_randomly(augmented_list)
    # else:
    return sorted(augmented_list)

def split_list_randomly(augmented_list):

    random.shuffle(augmented_list)
    
    n = len(augmented_list)
    size = n // 3

    segment = augmented_list[:size]
